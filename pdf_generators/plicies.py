...
def create_pdf(data):
    pdf = pdf_header("Our Company Policies")

    pdf.set_fill_color(120, 120, 120)
    pdf.set_draw_color(220, 220, 220)

    add_header(pdf, " Please Note: COVID-19 Policy Updates ")
    pdf.write(5.5,
    ...
    add_li(pdf,
           "Face masks and distancing are required. All guests are required to wear a face mask while in the indoor "
    ...
    add_li(pdf,
           "Daily housekeeping is temporarily suspended. In accordance with our updated COVID-19 related procedures and"
    ...
    add_header(pdf, "Cancellation and Change Policy")
    ...
    add_header(pdf, "Cancellation periods and fees")
    ...
    add_header(pdf, "Child Policies")

    add_li(pdf, "FOC for children under 5 years and below.")
    add_li(pdf, "Children between 6 to 11 years will be charged Rs.2500 of the room rate.")
    add_li(pdf, "Children 12 years and above are considered as an adult & will be charged the full rate.")
    ...
    for item_index, item in data.items():
        if "_title" in item_index:
            value = item.get()
            if value != "":
                add_header(pdf, value)
        else:
            if len(item.get('1.0', 'end-1c')) != 0:
                pdf.write(5.5, item.get("1.0", "end"))

    pdf.ln(6)

    pdf_footer(pdf)

    pdf_output(pdf, "policies")
